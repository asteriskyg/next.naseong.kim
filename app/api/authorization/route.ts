...
export async function GET(req: NextRequest) {
  ...
  if (!process.env.NEXT_APP_HOST) return NextResponse.json({ error: 'NEXT_APP_HOST is not defined.' })
  ...
  if(!tokenResponse.ok) return NextResponse.redirect(`https://${process.env.NEXT_APP_HOST}`);

  const token = await tokenResponse.json();
  const res = NextResponse.redirect(`https://${process.env.NEXT_APP_HOST}`);
